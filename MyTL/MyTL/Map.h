...
#define  BLACK false
	...
	public:
		...
		void ForEachlevelOrder(std::function<void(T0, T, int)>func);
	...
	template<typename T0, typename T>
	inline void Map<T0, T>::ForEachlevelOrder(std::function<void(T0, T, int)> func)
	{
		using MyTL::Stack;
		Stack<Node_map<T0, T>*> floor_curr;
		floor_curr.Push(root_node);
		int depth=0;
		while (!floor_curr.is_empty())
		{
			Stack<Node_map<T0, T>*> floor_next;
			for (auto i : floor_curr)
			{
				if (i)
				{
					func(i->key, i->value, depth);
					if (i->left)
					{
						floor_next.Push(i->left);
					}
					if (i->right)
					{
						floor_next.Push(i->right);
					}
				}
			}
			floor_curr = floor_next;
			depth++;
		}
	}
